package com.github.cache.redis.lock;
...
public class RedisLock {
     ...
     * 默认请求锁的超时时间(ms 毫秒)
     */
    private static final long TIME_OUT = 100*10*100;

    /**
     * 默认锁的有效时间(s)
     */
    public static final int EXPIRE = 60 * 1000;
...
//        sb.append("    return \"ok\"  else return \"error1\" ");
        sb.append("else ");
        sb.append("    return \"error2\" ");
        sb.append("end ");
...
//                        });
                    ...
                    if (result == 1) {
                        logger.info("Redis分布式锁，解锁{}成功！解锁时间：{}", lockKey, System.currentTimeMillis());
                        locked = false;
                        return true;
                    }
                    return false;
...
//                    RedisAsyncCommandsImpl;
                    ...
                    setArgs = SetArgs.Builder.ex(seconds).nx();

                    RedisFuture future = commands.set(key.getBytes(), value.getBytes(), setArgs);
                    try {
